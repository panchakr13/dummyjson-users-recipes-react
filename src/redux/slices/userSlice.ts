...
// import { loadAuthUsers, getUserById } from "../../services/api.service.ts";
//
// type UserSliceType = {
//     users: IUser[];
//     user: IUser | null,
    ...
    extraReducers: builder =>
        builder
            ...
            .addCase(searchUsersThunk.fulfilled, (state, action: PayloadAction<IUser[]>) => {
                // Замінюємо поточний список користувачів на результати пошуку
                state.users = action.payload;
            })
            .addMatcher(isFulfilled(loadUsers), (state) => {
                state.loadState = true;
...
export const userSliceActions = {
    ...
    loadUsers,
    loadUser,
    searchUsers: searchUsersThunk // експортуємо thunk пошуку
};
